...
import { FaArrowLeft, FaBookOpen, FaBuilding, FaCalendar, FaHeart, FaRegHeart } from "react-icons/fa";
...
export function BookDetails() {
  ...
  const [status, setStatus] = useState<BookReadingStatus>(BookReadingStatus.QUERO_LER);
  ...
  if (!book) {
    return (
      <div className="h-screen bg-dark flex items-center justify-center text-white">
        <p>Livro não encontrado. <span onClick={() => navigate('/myShelf')} className="text-primary cursor-pointer underline">Voltar.</span></p>
      </div>
    );
  }
  ...
  async function handleAddToShelf() {
    ...
    try {
      ...
      if (error.response?.data?.message?.includes("existe") || error.response?.status === 409) {
          ...
          <div className="bg-white rounded-lg shadow-2xl overflow-hidden flex flex-col md:flex-row">
            ...
            <div className="md:w-2/3 p-8 flex flex-col">
              <div className="flex justify-between items-start">
                <div>
                    <h1 className="text-4xl font-playfair font-bold text-dark mb-2 leading-tight">{book.title}</h1>
                    <h2 className="text-xl text-gray-600 font-medium mb-6">por <span className="text-primary">{book.author}</span></h2>
                </div>
                <button 
              ...
              <div className="grid grid-cols-2 gap-4 mb-6 text-sm text-gray-600 border-b border-gray-200 pb-6">
                <div className="flex items-center gap-2"><FaCalendar className="text-primary" /> <span>{book.publicationYear || "N/A"}</span></div>
                <div className="flex items-center gap-2"><FaBuilding className="text-primary" /> <span>{book.publisher || "N/A"}</span></div>
                <div className="flex items-center gap-2"><FaBookOpen className="text-primary" /> <span>{book.pageCount || "?"} pág.</span></div>
                <div className="flex items-center gap-2"><span className="font-bold text-primary">ISBN:</span> <span className="truncate w-24" title={book.isbn}>{book.isbn || "N/A"}</span></div>
              </div>
              ...
              <div className="bg-gray-50 p-4 rounded-md mb-6">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div className="flex flex-col">
                        <label className="text-xs font-bold text-gray-500 mb-1">Páginas Lidas</label>
                        <input 
                            type="number" 
                            value={pagesRead}
                    ...
                    <div className="flex flex-col">
                        <label className="text-xs font-bold text-gray-500 mb-1">Início</label>
                        <input 
                            type="date" 
                            value={startDate}
                    ...
                    <div className="flex flex-col">
                        <label className="text-xs font-bold text-gray-500 mb-1">Término</label>
                        <input 
                            type="date" 
                            value={finishDate}
